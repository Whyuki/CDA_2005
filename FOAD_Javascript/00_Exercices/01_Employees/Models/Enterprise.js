...
class Enterprise {
  ...
  constructor(_name) {
    ...
    if (fs.existsSync(this.filename)) {
      ...
      for (i = 0; i < json.length; i++) {
        // Parcours de la collection d'employées ainsi récupérée
  ...
  isValid(_employee) {
    if (!(_employee instanceof Employee)) {
      // Si "_employee" n'est pas une instacne de la classe "Employee"
  ...
  create(_employee) {
    ...
    if (
      ...
      this.employees.find((e) => e.id === _employee.id) === undefined
    ) {
  ...
  update(_employee) {
    ...
    if (exists !== undefined) {
      // si l'employé correspondant a été trouvé
  ...
  delete(_id) {
    ...
    if (toDelete > -1) {
      // Si un employé correspondant a été trouvé
